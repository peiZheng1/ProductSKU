...
@interface ProductSKUDataFilter () {
    ProductSKUCondition *_defaultSku;
}

@property (nonatomic, strong) NSSet <ProductSKUCondition *> *conditions;
...
@implementation ProductSKUDataFilter
...
- (void)initPropertiesSkuListData {
    ...
    for (int i = 0; i < [_dataSource numberOfConditionsInFilter:self]; i ++) {
        ...
        ProductSKUCondition *model = [ProductSKUCondition new];
    ...
    if (_defaultSku) {
        [_defaultSku.properties enumerateObjectsUsingBlock:^(ProductSKUProperty * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
//获取属性
- (NSArray<ProductSKUProperty *> *)propertiesWithConditionRawData:(NSArray *)data {
...
- (ProductSKUProperty *)propertyOfValue:(id)value inSection:(NSInteger)section {
    ...
    return [[ProductSKUProperty alloc] initWithValue:value indexPath:indexPath];
}
...
- (NSMutableSet<NSIndexPath *> *)getAllAvailableIndexPaths {
    ...
    [_conditions enumerateObjectsUsingBlock:^(ProductSKUCondition * _Nonnull obj, BOOL * _Nonnull stop) {
...
- (NSMutableSet<NSIndexPath *> *)availableIndexPathsFromSelctedIndexPath:(NSIndexPath *)selectedIndexPath sectedIndexPaths:(NSSet<NSIndexPath *> *)indexPaths{
    ...
    [_conditions enumerateObjectsUsingBlock:^(ProductSKUCondition * _Nonnull obj, BOOL * _Nonnull stop) {
        if ([obj.conditionIndexs objectAtIndex:selectedIndexPath.section].integerValue == selectedIndexPath.item) {
            ...
            [obj.properties enumerateObjectsUsingBlock:^(ProductSKUProperty * _Nonnull property, NSUInteger idx2, BOOL * _Nonnull stop1) {
...
#pragma mark -- setter
- (void)setDataSource:(id<ProductSKUDataFilterDataSource>)dataSource {
    _dataSource = dataSource;
    [self reloadData];
}
...
@implementation ProductSKUCondition

- (void)setProperties:(NSArray<ProductSKUProperty *> *)properties {
    ...
    [properties enumerateObjectsUsingBlock:^(ProductSKUProperty * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
@implementation ProductSKUProperty
